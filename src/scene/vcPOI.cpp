...
#pragma region NewIcion
    for (int i = 0; i < pProgramState->Icions.size(); i++)
    {
      if (strcmp(pProgramState->Icions[i].UUID, this->UUID) == 0)
      {
        pProgramState->Icions[i].worldPosition = m_attachment.currentPos;   
        break;
      }
    }
#pragma endregion
...
#pragma region NewIcion
    if (ImGui::BeginMenu(vcString::Get("sceneIcionAttachModel")))
    {
      static char uriBuffer[1024];
      static const char* pErrorBuffer;
      ...
      if (ImGui::Button(vcString::Get("sceneIcionAttachModel")))
      {
        for (size_t i = 0; i < pProgramState->Icions.size(); i++)
        {
          if (strcmp(pProgramState->Icions[i].m_pLoadedURI, uriBuffer) == 0)
          {
            for (int j = 0; j < 37; ++j)
              this->UUID[j] = pProgramState->Icions[i].UUID[j];
          }
        }
      }
