...
extern udChunkedArray<IcionDatas> IcionDatasH;
...
void vcModals_ModalsInformation(vcState* pProgramState)
{
  if (pProgramState->openModals & (1 << vcMT_ModelInformation))
    ImGui::OpenPopup(vcString::Get("menuModalsInformations"));

  ImGui::SetNextWindowSize(ImVec2(800, 600), ImGuiCond_Appearing);
  int Index = 0;
  if (ImGui::BeginPopupModal(vcString::Get("menuModalsInformations"), nullptr, ImGuiWindowFlags_NoResize))
  {
    pProgramState->modalOpen = true;
    ImGui::Columns(2, NULL, false);
    ...
    {
      if (IcionDatasH[i].IfShow)
      {     
       /* char* pBuf = udAllocType(char, 50, udAF_None);
        for (int j = 0; j < 30; ++j)
        {
          if (j < strlen(IcionDatasH[i].IcionInformation))
          {
            pBuf[j] = IcionDatasH[i].IcionInformation[j];
            pBuf[10] = '\0';
          }
        }*/

        ImGui::Text("%s", IcionDatasH[i].IcionInformation);
      }
    }
...
void vcModals_DrawModals(vcState *pProgramState)
{
  ...
  vcModals_ModalsInformation(pProgramState);
