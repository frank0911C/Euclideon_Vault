...
struct IcionDatas
{
  bool IfShow = false;
  const char* IcionURL;
  const char* IcionName;
  const char* IcionInformation;
};
udChunkedArray<IcionDatas> IcionDatasH;
extern udJSON tempIcionInformation;
int IfDownLoad = 0;
...
#pragma region NewIcion
  udJSONArray* pFeeds = nullptr;
  if (1 == IfDownLoad)
  {
    pFeeds = tempIcionInformation.Get("feeds").AsArray();
    bool result;
    if (nullptr != pFeeds)
    {
      UD_ERROR_CHECK(IcionDatasH.Init(32));//!!!!
      pProgramState->IcionDatas.Clear();

      for (size_t i = 0; i < pFeeds->length; ++i)
      {
        udJSON* pNode = pFeeds->GetElement(i);
        IcionDatas temp;
        temp.IcionURL = pNode->Get("feedURL").AsString();
        temp.IcionName = pNode->Get("name").AsString();
        temp.IcionInformation = pNode->Get("Information").AsString();
        IcionDatasH.PushBack(temp);
      }
      IfDownLoad = 0;
    }
  }
epilogue:
  for (size_t i = 0; i < pProgramState->Icions.size(); i++)
  {
    /* if (!Icions[i].IfVisible)
       continue;*/
    vcState::IcionArea Icions = pProgramState->Icions[i];
    ...
    udDouble4 s = udDouble4::create(Icions.worldPosition, 1.0);
    udDouble4 screenPosition = pProgramState->camera.matrices.viewProjection * s;
    ...
    ImVec2 windowPosition = ImVec2(float(screenPosition.x * pRenderContext->sceneResolution.x) + windowPos.x, float(screenPosition.y * pRenderContext->sceneResolution.y) + windowPos.y);

    //Control the display
    if (windowPosition.x < 0 || windowPosition.x > pRenderContext->sceneResolution.x ||
      windowPosition.y < 0 || windowPosition.y > pRenderContext->sceneResolution.y)
      continue;
    double distanceSq = udMagSq3(Icions.worldPosition - pProgramState->camera.position);
    if (Icions.Size.x != 32 || Icions.Size.y != 32)
    {
      Icions.Size.x = 64;
      Icions.Size.y = 64;
    }

    ImVec2 IcionSize = Icions.Size;

    //Control size
   /* if (distanceSq > (double)200 * (double)200)
    {
      continue;
    }
    else
    ...
    Icions.Position.x = windowPosition.x - IcionSize.x;
    Icions.Position.y = windowPosition.y + IcionSize.y*1.5;
    Icions.Area1.x = windowPosition.x - IcionSize.x;
    Icions.Area1.y = windowPosition.y;
    Icions.Area4.x = windowPosition.x;
    Icions.Area4.y = windowPosition.y + IcionSize.y * 1.5;

    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0)); 
    ImGui::SetNextWindowPos(ImVec2(Icions.Position.x, Icions.Position.y), ImGuiCond_Always, ImVec2(0.0f, 1.0f));
    ImGui::SetNextWindowSize(ImVec2((float)IcionSize.x, (float)IcionSize.y));
    ...
    strcpy(n_str, str);
    strcat(n_str, str2);
    if (ImGui::Begin(n_str, nullptr, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoNav))
      ImGui::Image(Icions.pTexture, ImVec2((float)IcionSize.x, (float)IcionSize.y), ImVec2(0, 0));

    ImGui::End();
    ImGui::PopStyleVar();

    //get Mouse
    if (io.MousePos.x > Icions.Area1.x&& io.MousePos.x < Icions.Area4.x &&
      io.MousePos.y > Icions.Area1.y&& io.MousePos.y < Icions.Area4.y)
    {
      if (io.MouseClicked[0])
      {
        for (size_t i = 0; i < IcionDatasH.length; ++i)
        {
          if (strcmp(IcionDatasH[i].IcionURL, Icions.m_pLoadedURI) == 0)
          {
            IcionDatasH[i].IfShow = true;
            pProgramState->openModals |= (1 << 11);
          }
        }
      }
      ...
      {
        vcLabelInfo temp = {};
        vcLabelInfo* tempp = &temp;

        for (size_t i = 0; i < IcionDatasH.length; ++i)
        {
          if (strcmp(IcionDatasH[i].IcionURL, Icions.m_pLoadedURI) == 0)
            tempp->pText = IcionDatasH[i].IcionName;
        }

        tempp->worldPosition = Icions.worldPosition;
        tempp->textColourRGBA = 4278190080;
        tempp->backColourRGBA = 4280059648;
        tempp->textSize = vcLFS_Large;
        vcLabelRenderer_Render(tempp, pProgramState->camera.matrices.viewProjection, pRenderContext->sceneResolution);
      }
    }
  }
#pragma endregion
