...
#pragma region NewIcion
        if (strstr(m_pLoadedURI, "Icon") == NULL)//The identification method needs to be modified
        {
          pRenderData->images.Insert(i, &pImageInfo);
        }
        else
        {
          if (m_pImageData != nullptr)
          {
            ...
            vcState::IcionArea temp;
            temp.worldPosition = m_image.position;
            for (int i = 0; i < 37; ++i)
              temp.UUID[i] = this->m_pNode->UUID[i];
            temp.Size.x = width;
            temp.Size.y = height;

            temp.pTexture = m_image.pTexture;
            temp.IfVisible = true;
            temp.m_pLoadedURI = this->m_pLoadedURI;
            pProgramState->Icions.push_back(temp);
          }
        }
#pragma endregion     
        udFree(m_pImageData);
